...
class DockerDriver(driver.ComputeDriver):
    ...
    def spawn(self, context, instance, image_meta, injected_files,
        ...
        if instance.architecture in ("arm", "armv7", "armv7l", "aarch64"):
            ...
            container_id = self._create_heterogenus_container(instance, image_name, args)
            if not container_id:
                raise exception.InstanceDeployFailure(
    ...
    def _create_container(self, instance, image_name, args):
        ...
        # zy: add volumes
        # volumes = args.pop('volumes', None)
        # volume_bindings = args.pop('volume_bindings', None)
        # host_config = docker_utils.create_host_config(binds=volume_bindings, **args)
    ...
    def _create_heterogenus_container(self, instance, image_name, args):
        name = "nova-" + instance['uuid']
        hostname = args.pop('hostname', None)
        cpu_shares = args.pop('cpu_shares', None)
        network_disabled = args.pop('network_disabled', False)
